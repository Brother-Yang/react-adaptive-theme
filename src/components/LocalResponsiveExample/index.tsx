...
import {
  useLocalResponsive,
  useLocalResponsiveText,
  useLocalResponsiveValue,
} from '../../hooks/useLocalResponsive';
import './index.less';
...
const LocalResponsiveExample: React.FC = () => {
  ...
  const buttonSize = useLocalResponsive(
    ...
    { defaultContent: 'middle' },
  ) as 'small' | 'middle' | 'large';
  ...
  const complexContent = useLocalResponsive(
    {
      md: (
        <div>
          <Paragraph>中等屏幕显示更多信息，包括详细的描述文本和多个操作按钮。</Paragraph>
          <Space>
  ...
  const dynamicContent = useLocalResponsive(
    {
      md: () => (
        <div>
          ...
          <br />
          <Text type='secondary'>当前时间: {new Date().toLocaleString()}</Text>
          <br />
          <Text type='warning'>屏幕宽度: {window.innerWidth}px</Text>
        </div>
      ),
      lg: () => (
  ...
  return (
    <div className='local-responsive-example'>
      <Card title='局部响应式组件示例' style={{ margin: spacing }}>
        {/* 响应式标题 */}
        <Title
          level={2}
          style={{
        ...
        {/* 使用说明 */}
        <Card type='inner' title='使用说明' size='small' style={{ marginTop: spacing }}>
          <Paragraph>
          ...
          <ul>
            <li>
              <Text code>useLocalResponsiveText</Text> - 专门处理文本内容
            </li>
            <li>
              <Text code>useLocalResponsiveValue</Text> - 专门处理数值内容
            </li>
          </ul>
        </Card>
      </Card>
