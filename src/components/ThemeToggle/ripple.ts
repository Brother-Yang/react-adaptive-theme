 ...
 */
export function computeMaxRadius(x: number, y: number): number {
 ...
 */
export function setupThemeClass(isDark: boolean): void {
...
export function triggerThemeRipple(
  ...
  if (!isViewTransitionSupported()) {
    onThemeChange();
    // 即使不支持 View Transition，也要设置主题类名
    setupThemeClass(!currentIsDark);
    return;
  }
