  ...
  return (
    <Card title={tAuto('测试翻译功能')} style={{ margin: '20px' }}>
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <div>
          <Title level={4}>{tAuto('当前语言')}: {locale}</Title>
        </div>
        ...
        <div>
          ...
          <Paragraph>
            {tAuto('这是手动指定key的文本', { key: 'test.manual.key' })}
          </Paragraph>
        </div>
        ...
        <div>
          ...
          <Paragraph>
            {tAuto('手动key的插值示例：用户 {{username}} 登录成功', { key: 'user.login.success', username: '李四' })}
          </Paragraph>
        </div>
