...
- 🛣️ **自动路由系统**: 基于文件系统的自动路由生成，支持嵌套路由和热更新
...
### 自动路由插件 (vite-plugin-auto-route)
- **文件系统路由**: 基于 `src/pages` 目录结构自动生成路由配置
- **嵌套路由支持**: 支持无限层级的嵌套路由结构
- **配置驱动**: 通过 `route.json` 文件配置路由元信息
- **热更新**: 路由配置文件变更时自动重新生成并热更新
- **TypeScript 生成**: 生成类型安全的 `.ts` 路由配置文件
- **React.createElement**: 使用函数式创建路由元素，避免 JSX 依赖
- **智能扫描**: 自动扫描页面目录，识别启用自动路由的页面
- **路径组合**: 自动处理父子路由路径组合和导入路径生成
...
2. 创建 `index.tsx` 作为页面组件
3. 创建 `route.json` 配置文件，设置路由信息：
```json
{
  "path": "/about",
  "name": "About",
...
```
4. 对于嵌套路由，在页面目录下创建子目录，重复上述步骤
5. 路由配置会自动生成到 `src/router/auto-routes.ts`
...
### 自动路由系统优化
- **🛣️ TypeScript 路由生成**: 路由配置文件从 `.tsx` 改为 `.ts` 格式，提升性能
- **⚡ React.createElement**: 使用函数式创建路由元素，避免 JSX 编译开销
- **🧹 配置简化**: 移除 `route.json` 中的 `description` 属性，简化配置结构
- **🔧 接口优化**: 更新插件接口定义，保持配置一致性
- **🚀 热更新增强**: 配置文件变更时自动重新生成路由并热更新
