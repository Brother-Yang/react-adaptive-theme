...
async function translateBatch(texts, sourceLang, targetLang) {
  ...
  console.log(
    `🔄 批量翻译 ${uncachedTexts.length} 个文本 (缓存命中: ${texts.length - uncachedTexts.length})`
  );
  ...
  for (let i = 0; i < processedTexts.length; i += batchSize) {
    ...
    console.log(
      `🔄 处理批次 ${Math.floor(i / batchSize) + 1}/${Math.ceil(
        processedTexts.length / batchSize
      )} (${batch.length} 个文本)`
    );
  ...
  try {
    ...
    for (const targetLang of targetLanguages) {
      ...
      if (updateInfo.isIncremental && updateInfo.newKeys.length > 0) {
        ...
        // 完整翻译
        translatedContent = await translateObject(defaultTranslations, targetLang, sourceLang);
      }
